...
let path = {
	build: {
		...
		css2: "../" + project_folder + "/css2/", //?указываем путь к CSS
		js: "../" + project_folder + "/js/",
		js2: "../" + project_folder + "/js2/",
		img: "../" + project_folder + "/img/",
		fonts: "../" + project_folder + "/fonts/",
		json: "../" + project_folder + "/js/",
		json2: "../" + project_folder + "/js2/",
	},
	src: {
		...
		css2: source_folder + "/scss/style2.scss", //?указываем путь к CSS
		js: source_folder + "/js/script.js",
		js2: source_folder + "/js/script2.js",
	...
	watch: {
		...
		css2: source_folder + "/scss/**/*.scss", //?слушаем все файлы CSS
		js: source_folder + "/js/**/*.js",
		js2: source_folder + "/js/**/*2.js",
		img: source_folder + "/img/**/*.{jpg,png,svg,gif,ico,jpeg,webp}",
		json: source_folder + "/js/**/.json",
	},
...
//? Создаем фукцию для работы с JSON

function json() {
	return src(path.src.json)
		.pipe(dest(path.build.json)) //? функция выгружет HTML файлы
		.pipe(dest(path.build.json2))
		.pipe(browsersync.stream()) //? Обновляет страницу
}
...
function css2() {
	return src(path.src.css2)
		...
		.pipe(webpcss()) //{webpClass: '.webp',noWebpClass: '.no-webp'}
		.pipe(dest(path.build.css2)) //? выгружаем обычный CSS файл, далее:
		.pipe(group_media({})) //? собирает все медиазапросы в одно место
		.pipe(clean_css({})) //? оптимизирует CSS файл
		.pipe(
			rename({
		...
		.pipe(dest(path.build.css2)) //? функция внутри кторой пишем команды для GULP
		.pipe(browsersync.stream()) //? Обновляет страницу
}
...
function js2() {
	return src(path.src.js2)
		.pipe(fileinclude()) //? Собирает JS файлы
		.pipe(dest(path.build.js2)) //? функция внутри кторой пишем команды для GULP
		.pipe(browsersync.stream()) //? Обновляет страницу
		.pipe(dest(path.build.js2)) //? выгружаем обычный JS файл, далее:
		.pipe(uglify({}))
		.pipe(
			rename({
				extname: ".min.js" //? выводит script.min.js
...
function watchFiles(params) { //? Создаем функцию которая будет следить за изменниями в файлах HTML
	...
	gulp.watch([path.watch.css2], css2) //? Создаем функцию которая будет следить за изменниями в файлах CSS
	gulp.watch([path.watch.js2], js2) //? Создаем функцию которая будет следить за изменниями в файлах JS
...
let build = gulp.series(clean, gulp.parallel(js, js2, css, html, css2, images,fonts, json), fontsStyle);
